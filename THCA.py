...
def read_THCA():
    ...
    set_size = pd.read_csv("THCA-groups.csv",sep = ",") 
...
class Stacked_SGL():
    ...
    def meta_MLR(self, X_test, Y_test):
    ...
    def Pred(self,X_test,Y_test):
        y_test, c = self.meta_MLR(X_test,Y_test)
        y_pre = meta_Pred(y_test)
        return y_pre 
    ...
    def Stacked_hoc(self, X_test,Y_test):
        y_test, coef2 = self.meta_MLR(X_test,Y_test)
    # def post_hoc(Coef_model,x_pos,x_neg,coef2): #测试集训练后再预测的结果
        ...
        sgl_coefs = sparse_group_lasso(np.vstack((self.x_pos,self.x_neg)), pred(y_hoc),0.01,
                                        0.8, self.groups, max_iter= 1000,verbose=True) #lambda越大越稀疏
        return sgl_coefs     
